...
class AGPTA_List_Table_Teams extends WP_List_Table {
	...
	public function get_sortable_columns(): array {
		$sortable_columns = array(
			'member_name' => array( 'member_name', true ),
			'member_role' => array( 'member_role', false ),
		);
	...
	public function process_bulk_actions(): void {
		...
		if ( 'bulk-delete' === $this->current_action() ) {
			...
			$ids = isset( $_POST['bulk-delete'] ) ? $_POST['bulk-delete'] : array();

			foreach ( $ids as $id ) {
				global $wpdb;

				$wpdb->delete( "{$wpdb->prefix}agpta_team", array( 'ID' => $id ) );
			}
		}
	}
